...
def _validate_vin(value: str) -> bool:
    """Validate VIN: exactly 17 alphanumeric characters."""
    v = (value or "").strip()
    return len(v) == 17 and v.isalnum()
...
class BMWCarDataConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        """Handle the start of the config flow: first ask for VIN."""
        return await self.async_step_vin(user_input)

    async def async_step_vin(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Ask for VIN; user can skip remaining steps if they already have a vehicle configured."""
        errors: dict[str, str] = {}

        if user_input is not None:
            vin_raw = (user_input.get(CONF_VIN) or "").strip()
            skip_credentials = user_input.get("skip_credentials", False)

            if skip_credentials:
                # User chose to use existing credentials; find an entry with valid client_id/gcid
                existing_entries = [
                    e
                    for e in self.hass.config_entries.async_entries(DOMAIN)
                    if (e.data.get(CONF_CLIENT_ID) or "").strip()
                    and (e.data.get(CONF_GCID) or "").strip()
                ]
                if not existing_entries:
                    errors["base"] = "no_existing_credentials"
                elif not _validate_vin(vin_raw):
                    ...
                    return self.async_create_entry(
                        ...
                        data={
                            ...
                            CONF_GCID: entry.data[CONF_GCID],
                            CONF_VIN: vin,
                            CONF_ID_TOKEN: entry.data.get(CONF_ID_TOKEN, ""),
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_VIN,
                        default=user_input.get(CONF_VIN) if user_input else "",
                    ): str,
                    vol.Optional("skip_credentials", default=False): bool,
                }
            ),
            errors=errors,
    ...
    async def async_step_device_flow(
        ...
        if user_input is not None:
            ...
            await self.async_set_unique_id(
                f"{self._gcid}_{self._vin}".lower()
            )
            self._abort_if_unique_id_configured()

            return self.async_create_entry(
                title=f"BMW CarData ({self._vin[:8]}…)",
                data={
